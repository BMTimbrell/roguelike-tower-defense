...
export default function level1(k: KAPLAYCtx) {
    k.scene("level1", async () => {
        ...
        mapData.layers.find(layer => layer.name === "Ground")?.data?.forEach((tile, index) => {
            k.add([
                k.pos(k.vec2(mapPosX + (index % mapData.width) * mapData.tilewidth, (Math.floor(index / mapData.height)) * mapData.tileheight)),
        ...
        )?.objects?.map(obj => k.vec2(mapPosX + obj.x, obj.y));
